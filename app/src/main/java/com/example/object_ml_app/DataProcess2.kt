...
class DataProcess2(val context: Context) {
...
//        const val LABEL_NAME = "yolov8n.txt"
    ...
    fun loadModel() {
        ...
        val outputFile = File(context.filesDir, FILE_NAME2) // 파일 경로 생성 시, "/" 대신 File.separator를 사용하는 것이 플랫폼 독립적

        assetManager.open(FILE_NAME2).use { inputStream ->
    ...
    fun loadLabel() {
        // txt 파일 불러오기
        BufferedReader(InputStreamReader(context.assets.open(LABEL_NAME2))).use { reader ->
    ...
    fun outputsToNPMSPredictions(outputs: Array<*>): ArrayList<Result> {
        val confidenceThreshold = 0.50f // 확률값의 임계치
